...
private class OpenSslConnection(private val ssl: CPointer<LYNSsl>?, private val context: CPointer<LYNSslCtx>?) : NativeTlsConnection {
...
actual fun nativeTlsProvider(): NativeTlsProvider = object : NativeTlsProvider {
    ...
    override fun server(clientFd: Int, certificatePath: String, privateKeyPath: String): NativeTlsConnection = memScoped {
        ...
        require(SSL_set_fd(ssl, clientFd) == 1); require(SSL_accept(ssl) == 1) { "TLS client handshake failed: ${sslError()}" }; OpenSslConnection(ssl, context)
    }
    override fun client(upstreamFd: Int): NativeTlsConnection = memScoped {
        ...
        require(SSL_set_fd(ssl, upstreamFd) == 1); require(SSL_connect(ssl) == 1) { "TLS upstream handshake failed: ${sslError()}" }; OpenSslConnection(ssl, context)
    }
}
