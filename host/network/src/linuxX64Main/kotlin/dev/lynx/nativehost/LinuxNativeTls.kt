...
private class OpenSslConnection(
    ...
    override fun read(maxBytes: Int): ByteArray? = memScoped { val buffer = allocArray<ByteVar>(maxBytes); val count = SSL_read(ssl, buffer, maxBytes); if (count <= 0) null else ByteArray(count) { buffer[it] } }
...
actual fun nativeTlsProvider(): NativeTlsProvider = object : NativeTlsProvider {
    ...
    override fun server(clientFd: Int, certificatePath: String, privateKeyPath: String, enableHttp2: Boolean): NativeTlsConnection = memScoped {
        ...
        require(SSL_set_fd(ssl, clientFd) == 1); require(SSL_accept(ssl) == 1) { "TLS client handshake failed: ${sslError()}" }; OpenSslConnection(clientFd, ssl, context, if (lynx_ssl_is_h2(ssl) == 1) "h2" else null)
    }
    override fun client(upstreamFd: Int, serverName: String, enableHttp2: Boolean): NativeTlsConnection = memScoped {
        ...
        if (enableHttp2) require(lynx_ssl_enable_h2_client(ssl) == 0) else require(lynx_ssl_enable_http1_client(ssl) == 0); require(SSL_connect(ssl) == 1) { "TLS upstream handshake failed: ${sslError()}" }; OpenSslConnection(upstreamFd, ssl, context, if (lynx_ssl_is_h2(ssl) == 1) "h2" else null)
    }
}
