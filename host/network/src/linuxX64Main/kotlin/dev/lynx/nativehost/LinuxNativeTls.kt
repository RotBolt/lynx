...
actual fun nativeTlsProvider(): NativeTlsProvider = object : NativeTlsProvider {
    ...
    override fun client(upstreamFd: Int, serverName: String, enableHttp2: Boolean): NativeTlsConnection = memScoped {
        ...
        if (enableHttp2) require(lynx_ssl_enable_h2_client(ssl) == 0); require(SSL_connect(ssl) == 1) { "TLS upstream handshake failed: ${sslError()}" }; OpenSslConnection(upstreamFd, ssl, context, if (lynx_ssl_is_h2(ssl) == 1) "h2" else null)
    }
}
