...
class NativeAndroidRelayController(private val processes: NativeProcessRunner) {
    data class Lease(val remotePath: String, val relayPid: String, val relayPort: Int)

    fun available(): Boolean = getenv("LYNX_ANDROID_RELAY_BINARY")?.toKString()?.let { path ->
        processes.run(listOf("test", "-x", path)).exitCode == 0
    } == true

    fun start(deviceSerial: String, packageName: String, pid: Int, hostPort: Int, captureToken: String): Lease {
        ...
        val remoteCommand = "chmod 700 '$remotePath' && setsid nohup '$remotePath' --package '${packageName.replace("'", "")}' --pid $pid --listen-port $relayPort --upstream-host 10.0.2.2 --upstream-port $hostPort --token '$captureToken' > '$remotePath.ready' 2>&1 < /dev/null & echo \$!"
        ...
        repeat(20) {
            ...
            if (ready.exitCode == 0 && ready.stdout.contains("relay_ready")) return Lease(remotePath, relayPid, relayPort)
            usleep(50_000u)
        }
        stop(deviceSerial, Lease(remotePath, relayPid, relayPort))
    ...
    fun stop(deviceSerial: String, lease: Lease?) {
        ...
        processes.run(prefix + listOf("shell", "pkill", "-f", lease.remotePath))
