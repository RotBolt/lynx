...
class PosixNativeCertificateAuthority(
    ...
    fun ensureCa(): String {
        ...
        if (!exists(ca) || !exists(key)) {
            ...
            require(result.exitCode == 0) { "unable to create native CA: ${result.stderr}" }
        }
        return ca
    }
    ...
    fun ensureLeaf(host: String): Leaf {
        ...
        if (!exists(cert) || !exists(key)) {
            ...
            require(runner.run(listOf("openssl", "req", "-new", "-newkey", "rsa:2048", "-nodes", "-keyout", key, "-out", csr, "-subj", "/CN=$host")).exitCode == 0) { "unable to create native leaf key" }
