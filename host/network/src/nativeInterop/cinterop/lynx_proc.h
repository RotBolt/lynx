...
static inline int lynx_proc_socket_owner(int pid, const char *remote_address, uint16_t remote_port) {
  ...
  for (int offset = 0; offset + (int)sizeof(struct proc_fdinfo) <= populated; offset += (int)sizeof(struct proc_fdinfo)) {
    ...
    char observed[INET6_ADDRSTRLEN] = {0};
    int family = 0;
    if (socket->insi_vflag == INI_IPV4) {
      family = AF_INET;
      if (!inet_ntop(family, &socket->insi_faddr.ina_46.i46a_addr4, observed, sizeof(observed))) continue;
    } else if (socket->insi_vflag == INI_IPV6) {
      family = AF_INET6;
      if (!inet_ntop(family, &socket->insi_faddr.ina_6, observed, sizeof(observed))) continue;
    } else continue;
    if (strcmp(observed, remote_address) == 0 && ntohs((uint16_t)socket->insi_fport) == remote_port) { result = 1; break; }
  }
  free(fds);
  return result;
}
